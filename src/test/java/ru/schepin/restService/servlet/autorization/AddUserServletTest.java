...
public class AddUserServletTest {
    ...
    public void whenCallDoGetThenServletReturnIndexPage() throws ServletException, IOException {
        ...
        when(request.getRequestDispatcher(Urls.addUser)).thenReturn(dispatcher); //var... args => OngoingStubbing<T> thenReturn(T value, T... values);

        servlet.doGet(request, response);

        verify(request, times(1)).getRequestDispatcher(Urls.addUser);
